...
import { Layout, Select, Button, Typography, message, ConfigProvider } from 'antd'; // Bỏ Card của AntD để dùng div thuần cho đẹp
...
import { 
  Utensils, Moon, Smile, Droplets, Dumbbell, 
  User, Calendar, Ruler, Weight, ChevronRight, ChevronDown,
  ChefHat, BookOpen, Apple, Popcorn, Activity
} from 'lucide-react';
...
// --- CONFIG & MOCK DATA ---
const CATEGORIES = [
  { id: 'eating', label: 'Ăn uống', icon: Utensils, color: 'text-emerald-500', bg: 'bg-emerald-50' },
  { id: 'sleep', label: 'Giấc ngủ', icon: Moon, color: 'text-indigo-500', bg: 'bg-indigo-50' },
  { id: 'mood', label: 'Cảm xúc', icon: Smile, color: 'text-yellow-500', bg: 'bg-yellow-50' },
  { id: 'water', label: 'Nước uống', icon: Droplets, color: 'text-blue-500', bg: 'bg-blue-50' },
  { id: 'activity', label: 'Vận động', icon: Dumbbell, color: 'text-orange-500', bg: 'bg-orange-50' },
];
...
const HealthConsultingPage: React.FC = () => {
  ...
  const handleUpdateProfile = async (values: any) => {
     ...
     if (!targetAccount) { message.error('Không tìm thấy hồ sơ'); return; }
     ...
     if (!isChanged) { message.info('Thông tin không có thay đổi'); return; }
     ...
     try {
        ...
        await toastPromise(
          ...
          { loading: 'Đang cập nhật...', success: 'Cập nhật thành công', error: 'Cập nhật thất bại' }
        );
  ...
  const handleCardClick = (id: number) => {
    ...
    if (id === 1) message.info('Tính năng đang phát triển');
  };

  const renderActiveView = () => {
    if (activeView === 'standards') return <NutritionalStandardsView healthDocumentId={Number(selectedAccountId)} onBack={() => setActiveView(null)} currentUser={currentAccount}/>;
    if (activeView === 'recommendations') return <FoodRecommendationsView healthDocumentId={Number(selectedAccountId)} onBack={() => setActiveView(null)} />;
    if (activeView === 'salt') return <SaltSugarAdviceView healthDocumentId={Number(selectedAccountId)} onBack={() => setActiveView(null)} />;
    return null;
  };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-sm text-gray-500 mb-2 font-medium">
              <span>Trang chủ</span> <ChevronRight size={14} /> <span className="text-emerald-600">Health Hub</span>
            </div>
            <h1 className="text-3xl md:text-4xl font-bold text-gray-800 tracking-tight">
              Trung tâm <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-teal-500">Tư vấn Sức khỏe</span>
            </h1>
            <p className="text-gray-500 mt-2 text-lg font-light">Lắng nghe cơ thể và nhận lộ trình tối ưu nhất.</p>
          </div>
        ...
        <div className="flex overflow-x-auto pb-4 gap-3 no-scrollbar md:justify-start">
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                key={cat.id}
                onClick={() => { setActiveTab(cat.id); setActiveView(null); }}
                className={`
                ...
                <div className="lg:w-1/3 flex flex-col gap-6 border-b lg:border-b-0 lg:border-r border-gray-100 pb-6 lg:pb-0 lg:pr-8">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                            <p className="text-xs font-bold text-emerald-600 uppercase tracking-wider mb-1">Hồ sơ đang xem</p>
                ...
                <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4">
                     ...
                     <div className="bg-orange-50 rounded-2xl p-4 flex flex-col justify-between relative overflow-hidden group hover:shadow-md transition-all">
                        ...
                        <div className="flex items-center gap-2 mb-2">
                             <div className="p-1.5 bg-white rounded-lg shadow-sm"><Weight size={16} className="text-orange-500" /></div>
                             <span className="text-xs font-bold text-orange-400 uppercase">Cân nặng</span>
                        </div>
                        <div>
                             <div className="text-2xl font-bold text-gray-800">
                                {currentAccount?.WEIGHT || '--'} <span className="text-sm font-medium text-gray-500">kg</span>
                             </div>
        ...
        <div className="min-h-[400px]">
          {activeTab === 'eating' ? (
            activeView ? (
              ...
              <div className="space-y-6 animate-fade-in">
                  <div className="flex items-center gap-2 mb-6">
                      ...
                      <h3 className="text-xl font-bold text-gray-800">Khám phá sức khỏe</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                      {RECOMMENDATIONS.map((item) => (
                          ...
                          >
                              ...
                              <div className={`absolute inset-0 bg-gradient-to-t ${item.gradient} opacity-80 group-hover:opacity-90 transition-opacity duration-300`} />
                              ...
                              <div className="absolute inset-0 p-6 flex flex-col justify-between text-white z-10">
                                  <div className="w-14 h-14 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center border border-white/30 group-hover:bg-white group-hover:text-emerald-600 transition-colors duration-300">
                                      <div className="group-hover:text-emerald-600 transition-colors">
                                        {/* Clone element để đổi màu icon khi hover nếu cần, hoặc để nguyên text-white */}
                                        {React.cloneElement(item.icon as React.ReactElement, { className: "group-hover:text-emerald-600 transition-colors" })}
                                      </div>
                                  </div>
                                  ...
                                  <div className="transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                                      ...
                                      <p className="text-white/80 text-sm font-medium flex items-center gap-2">
                                          {item.subtitle} <ChevronRight size={14} className="opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all" />
                                      </p>
                                  </div>
                              </div>
                          </div>
            ...
            <div className="flex flex-col items-center justify-center py-24 bg-white rounded-3xl border border-dashed border-gray-200">
              ...
              <h3 className="text-xl font-bold text-gray-400">Tính năng đang phát triển</h3>
              <p className="text-gray-400 mt-2">Đội ngũ kỹ thuật đang làm việc chăm chỉ!</p>
            </div>
          )}
        </div>
