...
type LiverFunctionChartProps = {
  ...
  type: LiverFunctionTabs;
  hiddenRange?: boolean;
};

export default function LiverFunctionChart({
  loading,
  type,
  conclusionList,
  hiddenRange
}: LiverFunctionChartProps) {
      ...
      case LiverFunctionTabs.SGOT:
        return [
          ...
          [
            ...
            {
              yAxis: getMaxValue(data, 5, 50, 10)
            }
          ]
        ];
  ...
  const yAxisGridLineData = useMemo(() => {
    return createSteppedArray(0, getMaxValue(data, 5, 50, 10), 5);
  }, [data]);
      ...
      formatter: function (params: any) {
        return `
              <div style="font-family: 'Roboto'; color: #333; font-size: ${isMobile ? '12px' : '18px'}; line-height: 140%;">
                  <div>${type === LiverFunctionTabs.SGOT ? 'SGOT/AST' : 'SGPT/ALT'}: <span style="font-weight: 700;">${convertDecimalDotToComma(params.data.value, 1)} U/L</span></div>
              </div>`;
      },
