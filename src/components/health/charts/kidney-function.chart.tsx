...
type KidneyFunctionChartProps = {
  loading: boolean;
  conclusionList: Conclusion[];
  type: KidneyFunctionTabs;
  hiddenRange?: boolean;
};

export default function KidneyFunctionChart({
  loading,
  type,
  conclusionList,
  hiddenRange
}: KidneyFunctionChartProps) {
      ...
      case KidneyFunctionTabs.Creatinine:
        return [
          ...
          [
            ...
            {
              yAxis: getMaxValue(data, 10, 150, 20)
            }
          ]
        ];
    }
  ...
  const yAxisGridLineData = useMemo(() => {
    return createSteppedArray(
      0,
      type === KidneyFunctionTabs.Ure
        ? getMaxValue(data, 1, 10, 2)
        : getMaxValue(data, 10, 150, 20),
      type === KidneyFunctionTabs.Ure ? 1 : 10
    );
  }, [data, type]);
      ...
      formatter: function (params: any) {
        return `
              <div style="font-family: 'Roboto'; color: #333; font-size: ${isMobile ? '12px' : '18px'}; line-height: 140%;">
                  <div>${type === KidneyFunctionTabs.Ure ? 'Ure' : 'Creatinine'}: <span style="font-weight: 700;">${convertDecimalDotToComma(params.data.value, 1)}${type === KidneyFunctionTabs.Ure ? ' mmol/L' : ' µmol/L'}</span></div>
              </div>`;
      },
    ...
    yAxis: {
      ...
      max:
        type === KidneyFunctionTabs.Ure
          ? getMaxValue(data, 1, 10, 2)
          : getMaxValue(data, 10, 150, 20),
      interval: type === KidneyFunctionTabs.Ure ? 1 : 10,
      axisLine: {
        show: true
      },
      axisLabel: {
