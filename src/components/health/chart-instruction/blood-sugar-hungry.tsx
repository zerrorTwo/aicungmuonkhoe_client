...
import BloodSugarDetailHungry from '../chart-instruction-detail/blood-sugar-hungry';

export default function BloodSugarHungry() {
  return (
    <div css={rootStyles}>
      ...
      <div css={detailGuideStyles}>
        {isMobile ? (
          <Collapse
            label='Hướng dẫn chi tiết đọc biểu đồ chỉ số đường huyết lúc đói'
            content={<BloodSugarDetailHungry />}
            boldActivatedLabel={false}
          />
        ) : (
          <div css={desktopStyles}>
            <div>Hướng dẫn chi tiết đọc biểu đồ chỉ số đường huyết lúc đói</div>
            <BloodSugarDetailHungry />
          </div>
        )}
      </div>
    </div>
