...
import LiverFunctionSGOTDetail from '../chart-instruction-detail/liver-function-sgot';

export default function LiverFunctionSGOT() {
  return (
    <div css={rootStyles}>
      ...
      <div css={detailGuideStyles}>
        {isMobile ? (
          <Collapse
            label='Hướng dẫn chi tiết đọc biểu đồ chỉ số SGOT/AST'
            content={<LiverFunctionSGOTDetail />}
            boldActivatedLabel={false}
          />
        ) : (
          <div css={desktopStyles}>
            <div>Hướng dẫn chi tiết đọc biểu đồ chỉ số SGOT/AST</div>
            <LiverFunctionSGOTDetail />
          </div>
        )}
      </div>
    </div>
