...
import BloodLipidLDLDetail from '../chart-instruction-detail/blood-lipid-ldl';

export default function BloodLipidLDL() {
  return (
    <div css={rootStyles}>
      ...
      <div css={detailGuideStyles}>
        {isMobile ? (
          <Collapse
            label='Hướng dẫn chi tiết đọc biểu đồ chỉ số Cholesterol loại LDL'
            content={<BloodLipidLDLDetail />}
            boldActivatedLabel={false}
          />
        ) : (
          <div css={desktopStyles}>
            <div>Hướng dẫn chi tiết đọc biểu đồ chỉ số Cholesterol loại LDL</div>
            <BloodLipidLDLDetail />
          </div>
        )}
      </div>
