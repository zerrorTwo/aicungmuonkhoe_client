...
import AcidUricDetail from '../chart-instruction-detail/acid-uric';
import { isMobile } from 'react-device-detect';

export default function AcidUric() {
  return (
    <div css={rootStyles}>
      ...
      <div css={detailGuideStyles}>
        {isMobile ? (
          <Collapse
            label='Hướng dẫn chi tiết đọc biểu đồ chỉ số Axit uric'
            content={<AcidUricDetail />}
            boldActivatedLabel={false}
          />
        ) : (
          <div css={desktopStyles}>
            <div>Hướng dẫn chi tiết đọc biểu đồ chỉ số Axit uric</div>
            <AcidUricDetail />
          </div>
        )}
      </div>
