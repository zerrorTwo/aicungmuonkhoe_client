...
import BloodLipidCholesterolDetail from '../chart-instruction-detail/blood-lipid-cholesterol';
import { isMobile } from 'react-device-detect';

export default function BloodLipidCholesterol() {
  return (
    <div css={rootStyles}>
      ...
      <div css={basicGuideInfoStyles}>
        ...
        <div>
          <span /> Cao hơn bình thường
        </div>
      </div>
      <div css={detailGuideStyles}>
        {isMobile ? (
          <Collapse
            label='Hướng dẫn chi tiết đọc biểu đồ chỉ số Cholesterol toàn phần'
            content={<BloodLipidCholesterolDetail />}
            boldActivatedLabel={false}
          />
        ) : (
          <div css={desktopStyles}>
            <div>Hướng dẫn chi tiết đọc biểu đồ chỉ số Cholesterol toàn phần</div>
            <BloodLipidCholesterolDetail />
          </div>
        )}
      </div>
    </div>
