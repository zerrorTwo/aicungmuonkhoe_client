...
import BloodLipidTriglycerideDetail from '../chart-instruction-detail/blood-lipid-triglyceride';
import { isMobile } from 'react-device-detect';

export default function BloodLipidTriglyceride() {
  return (
    <div css={rootStyles}>
      ...
      <div css={detailGuideStyles}>
        {isMobile ? (
          <Collapse
            label='Hướng dẫn chi tiết đọc biểu đồ chỉ số Triglyceride'
            content={<BloodLipidTriglycerideDetail />}
            boldActivatedLabel={false}
          />
        ) : (
          <div css={desktopStyles}>
            <div>Hướng dẫn chi tiết đọc biểu đồ chỉ số Triglyceride</div>
            <BloodLipidTriglycerideDetail />
          </div>
        )}
      </div>
    </div>
