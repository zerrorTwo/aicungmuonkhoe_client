...
export interface KidneyUpdatePayload {
    ID?: number;
    DATE: string;
    VALUE: number;
    HEALTH_DOCUMENT_ID: number;
    MODEL: string; // URE or CREA
    AGE_TYPE: string;
    TIME: string;
}
interface KidneyUpdateModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit?: (data: KidneyUpdatePayload) => Promise<void> | void;
    initialData?: Partial<KidneyUpdatePayload & { id?: number; ID?: number }> | null;
    healthDocumentId: number;
    ageType: string;
    activeTab: string;
}

const KidneyUpdateModal: React.FC<KidneyUpdateModalProps> = ({ isOpen, onClose, onSubmit, initialData, healthDocumentId, ageType, activeTab }) => {
    ...
    const validate = () => {
        ...
        if (!value || isNaN(v) || v <= 0) e.value = "Chỉ số thận không hợp lệ";
        setErrors(e);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const payload: KidneyUpdatePayload = {
                DATE: date,
    ...
    return (
        <ModalOverlay isOpen={isOpen} onClose={onClose}>
            <div className="p-6 max-h-[80vh] overflow-y-auto">
                <h2 className="text-xl font-bold text-foreground mb-4">Cập nhật chức năng thận</h2>
                <div className="space-y-4">
                    ...
                    <div>
                        <Label className="text-sm font-medium">Chỉ số <span className="text-red-500">*</span></Label>
                        <Input placeholder="Nhập chỉ số" value={value} onChange={(e) => setValue(e.target.value)} className="mt-1" />
...
export default KidneyUpdateModal;
