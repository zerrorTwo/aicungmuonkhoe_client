...
export interface UpdateHealthDocumentRequest extends CreateHealthDocumentRequest {
    id: number;
}
...
export const healthDocumentApi = baseApi.injectEndpoints({
    endpoints: (builder) => ({
        ...
        createHealthDocument: builder.mutation<HealthDocumentResponse, CreateHealthDocumentRequest>({
            query: (data) => ({
                url: '/health-document',
                method: 'POST',
                body: data,
        ...
        getHealthDocumentsByUserId: builder.query<HealthDocumentsListResponse, number>({
            query: (userId) => `/health-document/user/${userId}`,
            providesTags: ['HealthDocument'],
        }),

        // Update health document by ID
        updateHealthDocument: builder.mutation<HealthDocumentResponse, { id: number; data: UpdateHealthDocumentRequest }>({
            query: ({ id, data }) => ({
                url: `/health-document/${id}`,
                method: 'PUT',
                body: data,
            }),
            invalidatesTags: (_result, _error, { id }) => [{ type: 'HealthDocument', id }],
        }),

        // Delete health document by ID
        deleteHealthDocument: builder.mutation<{ success: boolean; message: string }, number>({
            query: (id) => ({
                url: `/health-document/${id}`,
                method: 'DELETE',
            }),
